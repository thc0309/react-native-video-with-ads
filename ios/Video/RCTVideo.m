...
- (instancetype)initWithEventDispatcher:(RCTEventDispatcher *)eventDispatcher
{
  if ((self = [super init])) {
    ...
    _mixWithOthers = @"inherit"; // inherit, mix, duck
      ...
      //Ads
      _settings = [[IMASettings alloc] init];
      _settings.enableDebugMode=NO;
      _adsLoader = [[IMAAdsLoader alloc] initWithSettings:_settings];
...
- (void)requestAds:(NSString *) adTagUrl {
    ...
    self->_adsLoader.delegate = self;
    IMAAdDisplayContainer *adDisplayContainer =
       [[IMAAdDisplayContainer alloc] initWithAdContainer:self viewController:self.firstAvailableUIViewController companionSlots:nil];
    // Create an ad request with our ad tag, display container, and optional user context.
    IMAAdsRequest *request = [[IMAAdsRequest alloc] initWithAdTagUrl:adTagUrl
                                                 adDisplayContainer:adDisplayContainer
                                                    contentPlayhead:self->_contentPlayhead
                                                        userContext:self];
    [self->_adsLoader requestAdsWithRequest:request];
    });
}
...
//    adsRenderingSettings.webOpenerDelegate = rootViewController ;
    adsRenderingSettings.linkOpenerPresentingController = rootViewController;
...
- (void)adsManager:(IMAAdsManager *)adsManager didReceiveAdEvent:(IMAAdEvent *)event {
    NSLog(@"event loading ads: %ld", (long)event.type);
...
- (void)setSrc:(NSDictionary *)source
{
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t) 0), dispatch_get_main_queue(), ^{
    ...
    [self playerItemForSource:self->_source withCallback:^(AVPlayerItem * playerItem) {
      ...
      self->_contentPlayhead = [[IMAAVPlayerContentPlayhead alloc] initWithAVPlayer:self->_player];
  });
